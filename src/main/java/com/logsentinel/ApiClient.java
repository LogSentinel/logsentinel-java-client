...
public class ApiClient {
    ...
    try {
      if (async) {
          ...
          if ("GET".equals(method)) {
              asyncResponse = invoker.get();
            } else if ("POST".equals(method)) {
              asyncResponse = invoker.post(entity);
            } else if ("PUT".equals(method)) {
              asyncResponse = invoker.put(entity);
            } else if ("DELETE".equals(method)) {
              asyncResponse = invoker.delete();
            } else if ("PATCH".equals(method)) {
              asyncResponse = invoker.method("PATCH", entity);
            } else if ("HEAD".equals(method)) {
              asyncResponse = invoker.head();
            } else {
              throw new ApiException(500, "unknown method type " + method);
            }
            return new FutureApiResponse<T>(asyncResponse, returnType);
      } else {
          if ("GET".equals(method)) {
  ...
  public class FutureApiResponse<T> extends ApiResponse<T> {
    private Future<Response> future;
    private GenericType<T> returnType;
    public FutureApiResponse(Future<Response> future, GenericType<T> returnType) {
        super(0, Collections.emptyMap());
        this.future = future;
    ...
    public T getData() {
        try {
            ...
            if (response.hasEntity()) {
              return ApiClient.this.deserialize(response, returnType);
            } else {
                return null;
            }
        } catch (Exception ex) {
    ...
    public Map<String, List<String>> getHeaders() {
        try {
            return ApiClient.this.buildResponseHeaders(future.get());
        } catch (Exception ex) {
            throw new RuntimeException(ex);
        }
    }
